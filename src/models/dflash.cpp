...
static ggml_tensor * build_dfly_ctx_out(llm_graph_context & g, const llama_model & model,
        ggml_tensor * inp, ggml_tensor * cur) {
    ggml_context * ctx0 = g.ctx0;
    const auto   & hparams = g.hparams;

    const int64_t n_embd   = g.n_embd;
    const int64_t n_tokens = g.n_tokens;
    const int64_t n_layer  = g.n_layer;
...
llama_model_dflash::graph<true>::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    if (model.dfly_layer_fusion) {
        cur = build_dfly_ctx_out(*this, model, inp, cur);
...
llama_model_dflash::graph<false>::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    if (ubatch.embd) {
        ...
        // the decoder owns the encoder now, so DFly's per-layer fusion has to happen here too
        ...
        if (model.dfly_layer_fusion) {
            inp_g = build_dfly_ctx_out(*this, model, inp_target, inp_g);
            cb(inp_g, "dfly_ctx_out", -1);
        } else {
            inp_g = build_norm(inp_g, model.output_norm_enc, NULL, LLM_NORM_RMS, -1);
            cb(inp_g, "inp_g_embeddings", -1);
        }

        ggml_tensor * inp_g_full = inp_g;
        ...
        res->t_embd = inp_g_full;

        ggml_build_forward_expand(gf, inp_g_full);
        return;
    }
